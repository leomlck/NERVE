...
# ---------------- Configuration ----------------
# Input directory where the RBC .pt files are stored.
s = 100
data_path = f'data/rbc-data-hr-{s}/'

# Output configuration: set the folder where you want to save the feature CSV
# and the desired CSV file name.
path_to_features = 'outputs/features'  # Change as needed.
#output_csv_filename = 'features_nerve_RBC_FC_FC_enc_FC_dec_nw_p1_RBC_avg.csv'
output_csv_filename = 'rs_fmri_triu_100_pnc.csv'

# Create the output directory if it does not exist.
os.makedirs(path_to_features, exist_ok=True)

# ----------------- Setup -----------------
# Regex pattern: expecting filenames like "sub-4211615439_fc.pt" to extract the subject ID.
...
triu_indices = torch.triu_indices(s, s, offset=1)
...
for i, fname in enumerate(files):
    ...
    # Convert to numpy array (make sure tensor is on CPU)
    upper_tri_np = upper_tri.cpu().numpy()

    # Create a dictionary with subject id and flattened features.
