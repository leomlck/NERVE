...
class MaskedAutoencoder(nn.Module):
    ...
    def patchify(self, fc_matrices, print_pad_fraction=False):
        """
        Extracts patches from FC matrices based on `network_list`.

        Args:
            fc_matrices (Tensor): FC matrices (B, N, N)
            return_mask (bool): whether to return a valid-entry mask
            print_pad_fraction (bool): optionally print padding fraction

        Returns:
            patches: Tensor (B, num_patches, max_patch_size)
            valid_masks (optional): Tensor (B, num_patches, max_patch_size)
        """
        ...
        for i, (net_a, idx_a) in enumerate(self.network_list.items()):
            for j, (net_b, idx_b) in enumerate(self.network_list.items()):
                ...
                patch = fc_matrices[:, idx_a][:, :, idx_b]   # (B, Na, Nb)
                patch_flat = patch.reshape(B, -1)            # (B, P)

                P = patch_flat.shape[1]
                pad_size = self.max_patch_size - P

                # pad values
                padded_patch = F.pad(patch_flat, (0, pad_size))

                # valid-entry mask (1 = real FC value, 0 = padding)
                valid_mask = torch.zeros(
                    (B, self.max_patch_size),
        ...
        patches = torch.stack(patches, dim=1)  # (B, num_patches, max_patch_size)
        valid_masks = torch.stack(masks, dim=1)

        if print_pad_fraction:
            pad_fraction = 1.0 - valid_masks.mean().item()
            print(f"[patchify] padding fraction: {pad_fraction:.3f}")

        return patches, valid_masks

    def forward_loss(self, fc_matrices, pred_patches, mask, use_valid_mask=True):
        """
        Computes masked MSE loss, ignoring padded values.

        Args:
            fc_matrices (Tensor): Ground-truth FC matrices (B, N, N)
            pred_patches (Tensor): Decoded patches (B, num_patches, max_patch_size)
            mask (Tensor): MAE mask (B, num_patches), 1 = masked
            use_valid_mask (bool): whether to ignore padded values

        Returns:
            Tensor: loss
        """
        target_patches, valid_masks = self.patchify(fc_matrices, print_pad_fraction=False)
        ...
        sq_error = (pred_patches - target_patches) ** 2

        if use_valid_mask:
            # remove padded entries from loss
            sq_error = sq_error * valid_masks
            denom = valid_masks.sum(dim=-1) + 1e-6
            patch_loss = sq_error.sum(dim=-1) / denom
        else:
            patch_loss = sq_error.mean(dim=-1)

        # MAE-style masking: only masked patches contribute
